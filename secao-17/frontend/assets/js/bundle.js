...
var showErrorMessage = function (inputItem, msg) {
    var formFields = inputItem.parentElement;
    if (formFields) {
    ...
    if (!formFields)
        console.error('Nenhum form fields encontrado');
};
var hideErrorMessages = function (formItem) {
    formItem.querySelectorAll("." + SHOW_ERROR_MESSAGES).forEach(function (item) {
...
var checkEmail = function (item) {
    if (!isEmail_1.default(item.value))
        showErrorMessage(item, 'Email inválido');
};
var checkEqualPassword = function (item1, item2) {
    if (item1.value !== item2.value) {
        showErrorMessage(item1, 'Senhas não batem');
        showErrorMessage(item2, 'Senhas não batem');
    }
};
var shouldSendForm = function (formItem) {
    var isExist = formItem.querySelectorAll("." + SHOW_ERROR_MESSAGES).length > 0;
    if (isExist)
        return false;
    return true;
};
